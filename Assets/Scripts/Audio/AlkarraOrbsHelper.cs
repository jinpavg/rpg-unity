...
public class AlkarraOrbsHelper : MonoBehaviour {
    ...
    [DllImport("AlkarraOrbs", CallingConvention = CallingConvention.StdCall)]
    ...
    public static AlkarraOrbsHelper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("AlkarraOrbsHelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                AlkarraOrbsHelper helper = go.AddComponent<AlkarraOrbsHelper>();
                helper.PluginKey = key;
                return helper;
            }
            return go.GetComponent<AlkarraOrbsHelper>();
        }
    }
