...
public class AlkarraSoundHelper : MonoBehaviour {
    ...
    [DllImport("AlkarraSound", CallingConvention = CallingConvention.StdCall)]
    ...
    public static AlkarraSoundHelper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("AlkarraSoundHelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                AlkarraSoundHelper helper = go.AddComponent<AlkarraSoundHelper>();
                helper.PluginKey = key;
                return helper;
            }
            return go.GetComponent<AlkarraSoundHelper>();
        }
    }
