...
public class BugFootHelper : MonoBehaviour {
    ...
    [DllImport("BugFoot", CallingConvention = CallingConvention.StdCall)]
    ...
    public static BugFootHelper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("BugFootHelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                BugFootHelper helper = go.AddComponent<BugFootHelper>();
                helper.PluginKey = key;
                return helper;
            }
            return go.GetComponent<BugFootHelper>();
        }
    }
